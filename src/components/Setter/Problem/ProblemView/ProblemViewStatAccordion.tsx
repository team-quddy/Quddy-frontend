...
import { ProblemStatType } from "../../../../types/types";

interface Props {
  no: number;
  problem: ProblemStatType;
  total: number;
}

const ProblemViewStatAccordion = ({ no, problem, total }: Props) => {
  return (
    <ProblemViewStatComponent>
      <Accordion id={`problem-${no}`}>
        ...
        <div className="content">
          ...
          <div className="problem-info">
            {problem.isObjective ? "객관식" : "주관식"} ·{" "}
            <span>{Math.round((problem.correct / total) * 1000) / 10}%</span>
          </div>
...
const ProblemViewStatComponent = styled.div`
  & > div {
...
export default ProblemViewStatAccordion;
